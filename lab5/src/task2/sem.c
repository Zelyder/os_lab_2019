...
static uint64_t thread_work(uint64_t a, uint64_t b, uint64_t mod) {
  uint64_t result = 0;
  result = a * b;
...
static void factorial_computation(fac_thread_t* f) {
  printf("Thread: id=%lu from %lu to %lu\n", f->thread, f->begin, f->end - 1);
  for (uint64_t i = f->begin; i < f->end; i++) {
    //semafor lock
    sem_wait(&semaphore);
    res = thread_work(res, i, f->mod);
    printf("res is %lu :: i = %d :: thread = %lu\n", res, i,f->thread);
    //semafor unlock
    sem_post(&semaphore);
  }
}
...
int main(int argc, char* argv[]) {
  ...
  for (uint32_t i = 0; i < pnum; i++) {
    ...
    if (pthread_create(&thread_pool[i].thread, NULL, (void *)factorial_computation, (void*)&thread_pool[i]) != 0) {
